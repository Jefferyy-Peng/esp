...
int main(int argc, char **argv) {
    ...
    in_words_adj = round_up(q * 10 + p * 10 + 41 + c * ( q + p ) * t, VALUES_PER_WORD);
    out_words_adj = round_up(c, VALUES_PER_WORD);
    ...
    for(unsigned i = 0; i < 1; i++)
        for(unsigned j = 0; j < q * 10 + p * 10 + 41 + c * ( q + p ) * t; j++)
