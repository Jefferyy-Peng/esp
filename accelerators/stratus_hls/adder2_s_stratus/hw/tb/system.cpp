...
void system_t::config_proc()
{
    ...
    // Compute
    {
        ...
        ESP_REPORT_TIME(begin_time, "BEGIN - adder2_s");
        ...
        ESP_REPORT_TIME(end_time, "END - adder2_s");
...
void system_t::load_memory()
{
    ...
    out_words_adj = size;
#else
    in_words_adj = round_up(2 * size, DMA_WORD_PER_BEAT);
    out_words_adj = round_up(size, DMA_WORD_PER_BEAT);
#endif
    ...
    for (int i = 0; i < 1; i++)
        for (int j = 0; j < size; j++)
            gold[i * out_words_adj + j] = (int32_t) j + (j + size);
