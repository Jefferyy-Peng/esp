...
#include "adder2_s_functions.hpp"

// Processes

void adder2_s::load_input()
{
...
#else
            uint32_t length = round_up(2 * size, DMA_WORD_PER_BEAT);
#endif
            // Chunking
...
void adder2_s::store_output()
{
    ...
    // Store
    {
        ...
        bool ping = true;
#if (DMA_WORD_PER_BEAT == 0)
        uint32_t store_offset = (2 * size) * 1;
#else
        uint32_t store_offset = round_up(2 * size, DMA_WORD_PER_BEAT) * 1;
#endif
        ...
        for (uint16_t b = 0; b < 1; b++)
        {
            ...
            for (int in_rem = in_length; in_rem > 0; in_rem -= PLM_IN_WORD)
            {
                ...
                for (int i = 0; i < in_len; i++) {
                    if (ping)
                        plm_out_ping[i] = plm_in_ping[i] + plm_in_ping[i + size];
                    else
                        plm_out_pong[i] = plm_in_pong[i] + plm_in_pong[i + size];
                }
